...
class QLRout(repo: AbstractRepo, userRepo: AbstractUserRepo) {
    ...
    extractCredentials { httpCredentials =>
      ...
      (post & path ( "graphql" )) {
        def process(authToken: Option[String]) = entity ( as [JsValue] ) { requestJson ⇒
          ...
          // https://github.com/OlegIlyenko/sangria-auth-example/blob/master/src/main/scala/Server.scala
              ...
              Tracer.withNewContext ( "graphql___query", autoFinish = true ) {
                complete {
                  Executor.execute (
                    schema = leomaxSchema,
                    queryAst = queryAst,
                    userContext = SecureContext ( authToken, userRepo, repo ),
                    variables = vars,
